...
이후로는 **매일 한국시간 오전 8시 30분**에 자동으로 실행됩니다 (밀리의서재가
자체 랭킹을 오전 7시 50분경에 갱신하는 걸 확인해서, 그보다 늦게 돌도록 여유를
뒀습니다 — 너무 이르면 전날과 같은 스냅샷을 두 번 긁게 됩니다).
(더 이르게/늦게 하고 싶으면 `.github/workflows/crawl.yml` 파일의
`cron: '30 23 * * *'` 부분의 숫자를 바꾸면 됩니다 — UTC 기준 시간이라 한국시간보다
9시간 느립니다)
